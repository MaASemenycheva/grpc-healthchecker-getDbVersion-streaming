...
class HealthCheckServer(private val port: Int) {
...
//                config.jdbcUrl = "jdbc:postgresql://localhost/test"
        ...
        override suspend fun getVersion(request: VersionRequest) = versionReply {

            val currentVersion = getVersion ()
            message = if (request.version.toInt() >= currentVersion.toInt()) {
                logger.info("Версия = ${request.version} >= минимальной требуемой текущей версии = $currentVersion СХД (SUCCESS)")
                "Версия = ${request.version} >= минимальной требуемой текущей версии = $currentVersion СХД (SUCCESS)"
            } else {
                logger.info("Версия = ${request.version} < минимальной требуемой текущей версии = $currentVersion  СХД (FAIL)")
                "Версия = ${request.version} < минимальной требуемой текущей версии = $currentVersion  СХД (FAIL)"
